...
class VMWriter(DummyWriter):
    ...
    def _output_dirname(self, path: str) -> str:
        parts = os.path.normpath(path).split(os.sep)
        if "shapenet" in parts:
            idx = parts.index("shapenet")
            return os.path.join(self.save_dir, *parts[idx:-1])
        return os.path.join(self.save_dir, os.path.dirname(path))
    ...
    def write(self, batch, prediction: List[Dict], dataset_module=None):
        ...
        for i, asset in enumerate(batch['asset']):
            ...
            dirname = self._output_dirname(path)
