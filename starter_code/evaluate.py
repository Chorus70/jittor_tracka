...
def filter_samples(samples, sample_list):
    if sample_list is None:
        return samples
    with open(sample_list, 'r', encoding='utf-8') as f:
        keys = [line.strip() for line in f if line.strip()]
    return {key: samples[key] for key in keys if key in samples}
...
def main():
    ...
    pred_samples = filter_samples(pred_samples, args.list)
    gt_samples = filter_samples(gt_samples, args.list)
    noisy_samples = filter_samples(noisy_samples, args.list)
    mesh_samples = filter_samples(mesh_samples, args.list) if use_p2s else {}
